@extends('layouts.plantilla')

@section('title', 'Inicio')

@section('contenido')

   <div class="container mt-5 text-center">
        <h1>{{ __('Noticia Literaria') }}</h1>
        <p>{{ __('El Día de las Bibliotecas se celebra cada 24 de octubre desde el año 1997. Este día nació como iniciativa de la Asociación
             Española de Amigos del Libro Infantil y Juvenil con el objetivo de concienciar a la sociedad de la importancia de la lectura
              y como homenaje y reconocimiento a la labor de los bibliotecarios/as.') }}</p>
              <img alt="imagen aleatoria" src="https://picsum.photos/700/400?random">
    </div>
    <div class="d-flex justify-content-center">
        <a href="{{ route('rutaregistro') }}" class="btn btn-success mt-3 w-50 text-center">{{ __('Ir a Registro') }}</a>
    </div>

@endsection
